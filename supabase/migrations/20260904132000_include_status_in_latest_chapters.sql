-- Migration: Include status in get_series_with_latest_chapters and ensure Sichuan chapters 40, 41, 42 are scheduled
...
-- Ensure latest chapters 40, 41, 42 of Sichuan Tyrant are scheduled with an active hold
UPDATE public.chapters
SET scheduled_at = (NOW() + INTERVAL '2 hours'),
    status = 'scheduled'
WHERE series_id = (SELECT id FROM public.series WHERE slug = 'sichuan-overlord' LIMIT 1)
  AND chapter_number IN (40, 41, 42);
