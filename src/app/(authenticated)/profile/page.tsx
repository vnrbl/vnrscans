...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const refreshUserData = () => {
      ...
      qc.invalidateQueries({ queryKey: ["profile", "reading-stats"] });
      qc.invalidateQueries({ queryKey: ["profile", "reading-history-dates"] });
      qc.invalidateQueries({ queryKey: ["profile", "reading-heatmap"] });
      qc.invalidateQueries({ queryKey: ["profile", "reading-preferences"] });
      qc.invalidateQueries({ queryKey: ["profile", "comment-history"] });
      qc.invalidateQueries({ queryKey: ["profile", "equipped-badge"] });
      qc.invalidateQueries({ queryKey: ["user-badges"] });
      qc.invalidateQueries({ queryKey: ["xp-history"] });
    };

    const channel = supabase
      .channel(`profile-me-realtime-sync-${userId}`)
      .on(
        "postgres_changes",
      ...
      .on(
        ...
        () => {
          qc.invalidateQueries({ queryKey: ["profile", "uploaded-series"] });
        }
      )
      .subscribe();
  ...
  const parentCommentsInfo = useQuery({
    queryKey: ["profile", "parent-comments", parentCommentIds.join(",")],
    queryFn: async () => {
  ...
  const readingPreferences = useQuery({
    ...
    queryFn: async () => {
      ...
      // Step 2: Get genre links for those series (in batches of 50)
      const allSgData: any[] = [];
