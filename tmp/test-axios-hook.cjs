...
(async () => {
  ...
  console.log('Landed on chapter page. Testing window.__comixAxios...');
  await new Promise(r => setTimeout(r, 2000));

  const result = await page.evaluate(async () => {
    if (!window.__comixAxios) return { error: 'window.__comixAxios not defined' };
    try {
      // Direct call to /chapters/11312305
      const res = await window.__comixAxios.get('/chapters/11312305');
  ...
  console.log('Direct Axios fetch result:', JSON.stringify(result, null, 2).slice(0, 1000));
