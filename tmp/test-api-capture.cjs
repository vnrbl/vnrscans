...
(async () => {
  ...
  page.on('response', async res => {
    ...
    if (u.includes('/api/v1/chapters/')) {
      try {
        ...
        console.log('[RAW API BODY]:', text.slice(0, 400));
        try {
          const json = JSON.parse(text);
          const p = json.result?.pages || json.pages;
          if (p) {
            apiPages = p;
            console.log('[PARSED PAGES]: baseUrl =', p.baseUrl, 'count =', p.items?.length);
          }
        } catch (e) {}
      } catch (e) {}
    }
  ...
  console.log('Done! apiPages items:', apiPages?.items?.length);
  await browser.close();
})();
