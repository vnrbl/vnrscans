...
(async () => {
  ...
  page.on('response', async res => {
    const u = res.url();
    if (u.includes('/api/v1/manga')) {
      try {
        console.log('API MANGA RES:', (await res.text()).slice(0, 300));
      } catch (e) {}
    }
  });

  await page.goto('https://comix.to/', { waitUntil: 'networkidle2' });
  ...
  console.log('Homepage loaded. Finding search input...');
  const searchEl = await page.$('input[type="search"], input[placeholder*="Search"], input');
  if (searchEl) {
    console.log('Typing Northern Blade...');
    await searchEl.type('Northern Blade', { delay: 100 });
    await new Promise(r => setTimeout(r, 3000));
    const results = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('a[href*="/title/"]')).map(a => ({ href: a.href, text: a.innerText.trim() }));
    });
