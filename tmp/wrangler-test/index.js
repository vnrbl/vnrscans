...
var index_default = {
  async fetch(request, env, ctx) {
    ...
    if (pathname === "/assets/upload" && request.method === "POST" && r2) {
      try {
        const uploadSecret = env.WORKER_UPLOAD_SECRET;
        if (uploadSecret) {
        ...
        await r2.put(body.key, buffer, {
          httpMetadata: body.contentType ? { contentType: body.contentType } : void 0,
    ...
    if (pathname === "/admin/recent-contacts" && env.RATE_LIMIT) {
      try {
        ...
        return Response.json({ contacts: items.filter(Boolean).sort((a, b) => b.ts.localeCompare(a.ts)) });
      } catch (e) {
